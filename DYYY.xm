...
- (void)reloadChannelWithChannelModels:(id)arg1 currentChannelIDList:(id)arg2 reloadType:(id)arg3 selectedChannelID:(id)arg4 {
    ...
    for (AWEHPTopTabItemUIModel *tabItemModel in channelModels) {
...
// 图文状态栏
%hook AWEFullPageFeedContainerViewController
- (BOOL)prefersStatusBarHidden {
    if (DYYYGetBool(@"DYYYHideStatusbar")) {
        return YES;
    }
    ...
    if (class_getInstanceMethod([self class], @selector(prefersStatusBarHidden)) !=
        class_getInstanceMethod([%c(AWEFullPageFeedContainerViewController) class], @selector(prefersStatusBarHidden))) {
        return %orig;
    }
    return NO;
}
...
%hook IESLLLivePurchaseAtmosphereView
- (void)layoutSubviews {
...
- (void)viewDidLayoutSubviews {
    ...
    if (!useFullHeight && [currentReferString isEqualToString:@"chat"]) {
        ...
        // 39.9.0 及更早版本的私信播放页以完整高度布局信息区，否则底部约束会整体上移。 （靠版本号判断不靠谱，这个是 abtest 的）
        if (currentVersion.length > 0 && [DYYYUtils compareVersion:currentVersion toVersion:@"39.9.0"] != NSOrderedDescending) {
            useFullHeight = YES;
        }
    }
